	...
	public void onMessageReactionAdd(MessageReactionAddEvent event) {
		...
		if (event.getChannelType() == ChannelType.TEXT) {
			if (!event.getUser().isBot()) {
				...
				try {
					if (set.next()) {
						long rollendid = set.getLong("rollenid");
						Guild guild = event.getGuild();
						guild.addRoleToMember(event.getMember(), guild.getRoleById(rollendid)).queue();
			...
			if (!event.getUser().isBot()) {
				...
				try {
					if (set.next()) {
						long rollendid = set.getLong("rollenid");
						Guild guild = event.getGuild();
						guild.removeRoleFromMember(event.getMember(), guild.getRoleById(rollendid)).queue();
